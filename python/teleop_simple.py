...
def main():
    ...
    linearVList=[0.25,0.5,0.75,-0.25,-0.5,-0.75,0,0,0,0]
    angularVList = [0,0,0,0,0,0,1,2,-1,-2]

    for i in range(5):

        for idx in range(len(linearVList)):
            cur_motor_command = mbot_motor_command_t()
            cur_motor_command.trans_v = 0.0
            cur_motor_command.angular_v = 0.0

            cur_motor_command.trans_v = linearVList[idx]
            cur_motor_command.angular_v = angularVList[idx]
    ...
    #         text = font.render('2', True, green, blue)
    ...
    #         text = font.render('-2', True, green, blue)
    #         cur_motor_command.trans_v = -0.5 # m/s
